...
<div class="@if($posts->count()<=1) w-1/2 @endif">
<div class="contenedor-cards">
    @foreach ($posts as $post )
    <div class="objeto-card">

        <h3 class="font-bold text-center text-orange-900 text-xl my-4">{{$post->title}}</h3>
        <h3 class="font-bold text-center text-gray-400 text-3xs my-2">{{$post->position}}{{$post->id}}</h3>
        ...
        @if($post->users?->count())
        <a href="{{route('admin-candidates',$post)}}" class="py-1 px-3 bg-green-400 rounded">See more..</a>
        @endif
        </div>
